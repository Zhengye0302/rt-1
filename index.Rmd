...
A statistical hypothesis is a value about a population parameter. Hypothesis testing uses stated hypotheses to make decisions.
...
The example data we are using can be downloaded from [Box](https://missouri.box.com/s/5an1r057v51ti2hmz5xbfp2qd41viers) or [Github](https://raw.githubusercontent.com/zhengyes/rt/master/rTutorialData.csv). These data are continuous and describe values of 5 types of biomarkers for 144 animals, by treatment by day. There are three categories of treatment type, treatment = 0, 5, 50. The biomarker values are measured on four days, day = 3, 6, 9 and 12. There is only one value for each treatment type and each day.
...
Here the F-test p-value = 0.2597 which is greater than 0.05, so we fail to reject the null hypothesis H0: ratio of the
...
The null hypothesis for the paired t-test is that the **mean difference of two groups** equals to 0. In this example, we will check biomarker KC for treatment = 0, to see whether or not biomarker KC has the same mean for day = 3 and day = 6. Because the data which we are going to compare for day = 3 and day = 6 are from the same subject, the paired t-test is appropriate. If the data are from two independent groups, you should use the two-sample t-test.
...
We use the Bartlett test to check equality of variances for all three groups at the same time. The null hypothesis H0 for Bartlett test is that all population variances are equal across groups.
...
#### Post hoc (Tukey)
The null hypothesis H0 for Tukey test is that all group means are equal.

The Tukey test p-value for 5-0 comparison is 0.5079363 > 0.05, so we fail to reject the null hypothesis H0. The p-value for 50-0 comparison is 0.0951873 > 0.05, so we fail to reject the null hypothesis H0. The p-value for 50-5 comparison is 0.0047412 < 0.05, so we reject the null hypothesis H0. The Tukey test reveals that the group means for treatment = 50 and treatment = 5 are significantly different. Because the lower bound and upper bound for the 95% confidence interval for the difference are both greater than 0 (lower bound (lwr) = 5.260493, upper bound (upr) = 35.086174), we can conclude that the group mean for treatment = 50 is greater than the group mean for treatment = 5. We also conclude that there is no significant difference between group means for neither treatment = 5 and treatment = 0, nor for treatment = 50 and treatment = 0.
```{r,eval=FALSE}
...
The Wilcoxon rank sum test is used to test whether two samples are likely derived from the same population (i.e., that the two populations have the same shape). You can also interpret this test as comparing the medians between the two populations. This test is a nonparametric alternative to the t-test for use when the assumption of normality is not valid. This test uses the ranks of the values rather than the values themselves. The null hypothesis is H0: two populations distributions are equal and they have the same median, the alternative hypothesis Ha is that the two population distributions are not equal and their medians are different.

**Example**: 

We want to compare the group means of CTXII for treatment = 5 and treatment = 50. The normality test shows that data are not normal, which means we cannot use the parametric method two-sample t-test. 

**Code**:

The Shapiro-Wilk test p-value for CTXII treatment = 50 = 0.0002087 < 0.05, so we reject the null Hypothesis that CTXII treatment =50 is normal, while the Shapiro-Wilk test p-value for treatment = 5 equals to 0.06939 >0.05, we fail to reject the null hypothesis H0 that treatment = 5 is normal. Since at least one group is non-normal, the normality assumption is violated, so we cannot use two-sample t-test and the Wilcoxon rank sum test will be used instead.
...
This is a nonparametric alternative to the paired t-test. The null hypothesis is H0: the median of the difference between pairs is 0. The alternative hypothesis Ha is that the median of the difference between the pairs is not 0.
...
Kruskal-Wallis test is a non-parametric alternative to a one-way ANOVA test, and is recommended when the assumptions of an ANOVA test are not met. The null hypothesis H0 is that, medians of all groups are equal, and the alternative hypothesis Ha is that at least one population median is different from the others.

**Example**:

We want to compare biomarker IL6 medians for treatment = 0, treatment = 5 and treatment = 50. If the Kruskal-Wallis test is significant, the post hoc Dunn test can be performed. The Dunn's test is a non-parametric pairwise multiple comparisons test based on rank sums, and it can help us to determine which group has a different median from each other group.

**Code**:

The Kruskal-Wallis p-value = 2.335e-06, which is less than 0.05. We conclude that not all groups have the same median, at least one group has a different median. 
```{r,eval=FALSE}
