...
This tutorial will help you to determine the best test for your data. The example data we are using can be downloaded at [Box](https://missouri.box.com/s/70v91qllsnax3fzwyci2gam30llnu7rh) or [Github](https://raw.githubusercontent.com/zhengyes/rt/master/rExampleData.csv). The continous data describe values of 6 kinds of biomarkers for 144 animals by treatment.
...
1. You can type the following code in console, then a "select file" window will pop up. Find the file you want to import and click "Open", the path will display in console window.

```{r eval=FALSE}
file.choose()
```

2. Now you have the path of the file, you can use read.csv function to read the .csv file into R. Type the following code in console but replace path between quotations with your file path. The new data set will be named "biomarker".
```{r}
biomarker = read.csv("C:\\Users\\zs7hm\\Desktop\\rExampleData.csv")
```

### Normality check
Many statistical tests require data to be normally distributed, this can be tested using a Shapiro normality test.

The null hypothesis of normality test is that there is no significant departure from normality. When the p-value is greater than 0.05, it fails to reject the null hypothesis and thus the assumption holds. Parametric methods (two sample t test, paired t test, ANOVA) need normality check, while the non-parametric methods (Wilcoxon rank sum, signed rank, Kruskal) can be used when data failed normality check.

Here is an example. We are going to use Shapiro test to check if biomarker MMP3 with treatment 0 is normally distributed. The null hypothesis of Shapiro test H0: data are normally distributed.
